...
//	userInteraction();
	pthread_t userInteractionThread;
	...
	for(int i=0 ; i<nThreads ; i++)
	{
		...
		pthread_create(&(children[i]),  //våra barn
			NULL,	//attributen av barnen
			frequencyAnalysis,	//functionen som ska köras
			&params[i]);	//argument för funktionen
...
//	frequencyAnalysis(params);
	...
	pthread_create(&userInteractionThread, NULL, userInteraction, NULL);
